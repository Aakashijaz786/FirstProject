...
// Fetch meta title, meta description, and active slug from how_pages and how_page_slugs for current language
...
// Get how_pages row for current language
$res = $conn->query("SELECT id, meta_title, meta_description, page_name FROM how_pages WHERE language_id=$current_lang_id LIMIT 1");
$how_page_id = null;
$page_name = '';
if ($res && $res->num_rows > 0) {
    $row = $res->fetch_assoc();
...
// Get active slug for this how_page_id
if ($how_page_id) {
    $res2 = $conn->query("SELECT slug FROM how_page_slugs WHERE how_page_id=$how_page_id AND status='active' LIMIT 1");
...
foreach ($languages as $lang) {
    ...
    // Get how_pages row for this language
    $res3 = $conn->query("SELECT id FROM how_pages WHERE language_id = $lang_id LIMIT 1");
    $how_page_id_alt = null;

    if ($res3 && $res3->num_rows > 0) {
        $row3 = $res3->fetch_assoc();
        $how_page_id_alt = $row3['id'];
    }

    // Get active slug for this how_page_id
    if ($how_page_id_alt) {
        $res4 = $conn->query("SELECT slug FROM how_page_slugs WHERE how_page_id = $how_page_id_alt AND status = 'active' LIMIT 1");
    ...
    // Add alternate only if there are multiple languages
    if ($language_count > 1) {
        $alternate_links[] = [
            'url' => $url,
            'hreflang' => $lang_code
...
<body class="bg-gray">
<?php
   file_put_contents('how_debug.log', 'how-download.php loaded: ' . date('c') . PHP_EOL, FILE_APPEND);
...
// Treat the entire slug as one unit (e.g., "es1/how-to-download-tiktok-video1")
if (!$lang) {
    // Search for the exact slug in how_page_slugs table
    $res = $conn->query("SELECT hps.how_page_id, l.* FROM how_page_slugs hps 
                         JOIN how_pages hp ON hps.how_page_id = hp.id 
                         JOIN languages l ON hp.language_id = l.id 
                         WHERE hps.slug = '" . $conn->real_escape_string($current_slug) . "' 
...
// Fetch the How page for this language
$how_page = null;
$active_how_slug = 'how';
if ($lang_id) {
    // Get the how_page for this language
    $res = $conn->query("SELECT * FROM how_pages WHERE language_id=$lang_id LIMIT 1");
    if ($res && $res->num_rows > 0) {
        $how_page = $res->fetch_assoc();
        $how_page_id = $how_page['id'];
        // Fetch the active slug from how_page_slugs
        $res2 = $conn->query("SELECT slug FROM how_page_slugs WHERE how_page_id=$how_page_id AND status='active' LIMIT 1");
        if ($res2 && $res2->num_rows > 0) {
            $row2 = $res2->fetch_assoc();
            $active_how_slug = $row2['slug'];
        } else {
            $active_how_slug = $how_page['slug'];
        }
    }
}
...
$page_header = h($how_page['header'] ?? 'How to Download TikTok Videos');
$page_heading = h($how_page['heading'] ?? 'Download TikTok videos online');
$page_description = $how_page['description'] ?? 'company name is a free TikTok download tool that helps you download TikTok videos without watermark online. Save TT videos with the highest quality in an MP4 file format and HD resolution. To find out how to use the TikTok watermark remover, follow the instructions below.';

// Fetch TikTok download instructions for the current language
$tiktok_instructions = [];

if ($lang_id) {
    $stmt = $conn->prepare("SELECT title, description FROM language_how_titles WHERE language_id = ?");
...
?>
            ...
            <div class="content-visibility">
               ...
               <!-- <section>
                        ...
                        <div class="main-c-c u-m-bottom d-flex">
                            ...
                            <h3><?php echo h($how_page['section1_title'] ?? ''); ?></h3>
                            <p><?php echo h($how_page['section1_description'] ?? ''); ?></p>
                            </div>
                            <div class="d-flex align-items-center text--image">
                                <div class="flex-1 img-w">
                                    <?php if (!empty($how_page['section1_image'])): ?>
                                        <img src="/uploads/how_sections/<?php echo htmlspecialchars($how_page['section1_image']); ?>" width="416" height="289" alt="Section 1 Image">
                                    <?php endif; ?>
                                </div>
                            </div>
                        </div>
                        <div class="main-c-c u-m-bottom d-flex">
                            <div class="d-flex text-pr align-items-center text--image">
                                <div class="flex-1 img-w">
                                    <?php if (!empty($how_page['section2_image'])): ?>
                                        <img src="/uploads/how_sections/<?php echo htmlspecialchars($how_page['section2_image']); ?>" width="368" height="321" alt="Section 2 Image">
                                    <?php endif; ?>
                                </div>
                            </div>
                            <div class="u-smaller-text flex-1 align-items-center text text--text">
                                <h3><?php echo h($how_page['section2_title'] ?? ''); ?></h3>
                                <p><?php echo h($how_page['section2_description'] ?? ''); ?></p>
                            </div>
                        </div>
                    </div>
                </section> -->
    ...
    <script src="/assets/js/how.js" defer></script>
   <?php
