...
// Try to find the contact page for this language
$page = null;
$sql = "SELECT * FROM language_pages WHERE language_id=$lang_id AND (LOWER(page_name) LIKE '%contact%' OR LOWER(slug) LIKE '%contact%') LIMIT 1";
$res = $conn->query($sql);
if ($res && $res->num_rows > 0) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $page_name = trim($_POST['page_name'] ?? 'Contact');
    $slug = trim($_POST['slug'] ?? 'contact');
...
?>
    <div class="main-content" id="mainContent">
        ...
        <div class="page-section">
            <h4>Contact Page (<?php echo htmlspecialchars($lang['name']); ?>)</h4>
            <div class="url-preview">URL: <b>/<?php echo htmlspecialchars($lang_code); ?>/<?php echo htmlspecialchars($page['slug'] ?? 'contact'); ?></b></div>
            <?php if ($success): ?>
            ...
            <form method="post">
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" name="page_name" value="<?php echo htmlspecialchars($page['page_name'] ?? 'Contact'); ?>">
                </div>
                <div class="mb-3">
                    <label class="form-label">Slug</label>
                    <input type="text" class="form-control" name="slug" value="<?php echo htmlspecialchars($page['slug'] ?? 'contact'); ?>">
                </div>
                <div class="mb-3">
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control" id="editor" name="content"><?php echo htmlspecialchars($page['content'] ?? ''); ?></textarea>
                </div>
