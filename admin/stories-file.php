...
// Fetch the Stories page for this language
$page = null;
$sql = "SELECT * FROM stories_pages WHERE language_id=$lang_id LIMIT 1";
$res = $conn->query($sql);
if ($res && $res->num_rows > 0) {
...
$english_page = null;
if (!$page && $lang_id != 41) {
    $sql = "SELECT * FROM stories_pages WHERE language_id=41 LIMIT 1";
    $res = $conn->query($sql);
    if ($res && $res->num_rows > 0) {
...
define('STORIES_DYNAMIC_TABLE', 'language_stories_titles');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['translate_content'])) {
    ...
    if ($english_page) {
        ...
        error_log("Translating Stories page from English to {$target_language}");
        ...
        $sql = "SELECT * FROM `".STORIES_DYNAMIC_TABLE."` WHERE language_id=41 ORDER BY id ASC";
        $res = $conn->query($sql);
        if ($res && $res->num_rows > 0) {
        ...
        $error = 'No English content found to translate. Please ensure English (ID 41) Stories page exists.';
    }
}

// Create table if not exists (constant already defined above)
if (!$conn->query("SHOW TABLES LIKE '".STORIES_DYNAMIC_TABLE."'")) {
    $conn->query("CREATE TABLE IF NOT EXISTS `".STORIES_DYNAMIC_TABLE."` (
...
if ($translated_data && isset($translated_data['titles'])) {
    $stories_titles = $translated_data['titles'];
} else {
    $stories_titles = [];
    $sql = "SELECT * FROM `".STORIES_DYNAMIC_TABLE."` WHERE language_id=$lang_id ORDER BY id ASC";
    $res = $conn->query($sql);
    if ($res && $res->num_rows > 0) {
        while ($row = $res->fetch_assoc()) {
            $stories_titles[] = $row;
...
$english_titles = [];
if (empty($stories_titles) && $lang_id != 41) {
    $sql = "SELECT * FROM `".STORIES_DYNAMIC_TABLE."` WHERE language_id=41 ORDER BY id ASC";
    $res = $conn->query($sql);
    if ($res && $res->num_rows > 0) {
        while ($row = $res->fetch_assoc()) {
...
if (empty($stories_titles)) {
    $stories_titles[] = ['id'=>0, 'title'=>'', 'description'=>''];
}

$slug = 'stories'; // default
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !isset($_POST['translate_content'])) {
    ...
    if (isset($_POST['delete_redirect_id'])) {
        ...
        // Only allow deleting inactive slugs from stories_page_slugs
        if ($conn->query("DELETE FROM stories_page_slugs WHERE id=$delete_id AND status='inactive'")) {
            echo json_encode(['success' => true]);
        } else {
            echo json_encode(['success' => false, 'error' => $conn->error]);
        }
        exit; // Crucial: exit immediately after handling AJAX delete
    }

    $page_id = isset($_POST['page_id']) ? intval($_POST['page_id']) : 0;
    $page_name = trim($_POST['page_name'] ?? 'stories');
    $slug = trim($_POST['slug'] ?? 'stories');
    ...
    if ($page_id) {
        // Update existing
        $sql = "UPDATE stories_pages SET page_name='" . $conn->real_escape_string($page_name) .
            ...
            "' WHERE id=$page_id";
        if ($conn->query($sql)) {
            $success = 'Stories page updated.';
        } else {
            $error = 'Failed to update Stories page: ' . $conn->error;
        }
    } else {
        // Insert new
        $sql = "INSERT INTO stories_pages (language_id, page_name, slug, meta_title, meta_description, meta_header, header, heading, description) VALUES ("
        ...
        if ($conn->query($sql)) {
            ...
            $error = 'Failed to add Stories page: ' . $conn->error;
        }
    }
    ...
    for ($i = 0; $i < count($titles); $i++) {
        ...
        if ($t && $d) {
            ...
            if ($tid && isset($existing_titles[$tid])) {
                $sql = "UPDATE `".STORIES_DYNAMIC_TABLE."` SET title='$t_esc', description='$d_esc' WHERE id=$tid AND language_id=$lang_id";
                $conn->query($sql);
                $handled_ids[] = $tid;
            } else {
                $sql = "INSERT INTO `".STORIES_DYNAMIC_TABLE."` (language_id, title, description) VALUES ($lang_id, '$t_esc', '$d_esc')";
                $conn->query($sql);
    ...
    foreach ($existing_titles as $id => $row) {
        if (!in_array($id, $handled_ids)) {
            $conn->query("DELETE FROM `".STORIES_DYNAMIC_TABLE."` WHERE id=$id");
        }
    }
    
    // Refresh stories_titles after saving to show updated values
    $stories_titles = [];
    $sql = "SELECT * FROM `".STORIES_DYNAMIC_TABLE."` WHERE language_id=$lang_id ORDER BY id ASC";
    $res = $conn->query($sql);
    if ($res && $res->num_rows > 0) {
...
if (isset($page['id']) && !empty($page['id'])) {
    $stories_page_id = $page['id']; // or $conn->insert_id after insert
    $new_slug = $slug;

    // Set all previous slugs for this page to inactive
    $conn->query("UPDATE stories_page_slugs SET status='inactive' WHERE stories_page_id=$stories_page_id");

    // Check if the new slug already exists
    $res = $conn->query("SELECT * FROM stories_page_slugs WHERE stories_page_id=$stories_page_id AND slug='$new_slug'");
    if ($res && $res->num_rows > 0) {
        // Update to active
        $conn->query("UPDATE stories_page_slugs SET status='active' WHERE stories_page_id=$stories_page_id AND slug='$new_slug'");
    } else {
        // Insert as active
        $conn->query("INSERT INTO stories_page_slugs (stories_page_id, slug, status) VALUES ($stories_page_id, '$new_slug', 'active')");
    }
} else {
    // You may also log or debug `$page` here
}
...
$redirects = [];
if ($page) {
    $res = $conn->query("SELECT * FROM stories_page_slugs WHERE stories_page_id=" . intval($page['id']) . " ORDER BY id ASC");
    while ($row = $res->fetch_assoc()) {
        $redirects[] = $row;
    }
}
...
<?php include 'includes/header.php'; ?>
    <div class="main-content" id="mainContent">
        ...
        <div class="page-section">
            <h4>Stories Page (<?php echo htmlspecialchars($lang['name']); ?>)</h4>
            <!--<div class="url-preview">URL: <b>/<?php echo htmlspecialchars($lang_code); ?>/<?php echo htmlspecialchars($page['slug'] ?? 'stories'); ?></b></div>-->
            <?php if ($success): ?>
            ...
            <form method="post">
                <div class="mb-3" style="display:none;">
                    ...
                    <input type="text" class="form-control" name="page_name" value="<?php echo htmlspecialchars($page['page_name'] ?? 'Stories'); ?>">
                </div>
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" name="slug" value="<?php echo htmlspecialchars($translated_data ? $translated_data['slug'] : ($page['slug'] ?? ($english_page['slug'] ?? 'download-tiktok-stories'))); ?>">
                    </div>
                    <div class="mb-3">
                ...
                <div class="mb-3">
                    ...
                    <div id="titlesContainer">
                        <?php 
                        $display_titles = $stories_titles;
                        $is_translated = $translated_data && isset($translated_data['titles']);
                        
                        if (empty($stories_titles) || (count($stories_titles) == 1 && empty($stories_titles[0]['title']))) {
    ...
    $stories_page_id = $page['id'] ?? 0;
    $slugs = [];
    if ($stories_page_id) {
        $res = $conn->query("SELECT * FROM stories_page_slugs WHERE stories_page_id=$stories_page_id ORDER BY id ASC");
        if ($res && $res->num_rows > 0) {
            while ($row = $res->fetch_assoc()) {
                $slugs[] = $row;
            }
