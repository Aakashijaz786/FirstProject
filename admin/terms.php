...
// Try to find the terms page for this language
$page = null;
$sql = "SELECT * FROM language_pages WHERE language_id=$lang_id AND (LOWER(page_name) LIKE '%terms%' OR LOWER(slug) LIKE '%terms%') LIMIT 1";
$res = $conn->query($sql);
if ($res && $res->num_rows > 0) {
...
$english_page = null;
if (!$page && $lang_id != 41) {
    $sql = "SELECT * FROM language_pages WHERE language_id=41 AND (LOWER(page_name) LIKE '%terms%' OR LOWER(slug) LIKE '%terms%') LIMIT 1";
    $res = $conn->query($sql);
    if ($res && $res->num_rows > 0) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['translate_content'])) {
    // Fetch English content if not already loaded
    if (!$english_page) {
        $sql = "SELECT * FROM language_pages WHERE language_id=41 AND (LOWER(page_name) LIKE '%terms%' OR LOWER(slug) LIKE '%terms%') LIMIT 1";
        $res = $conn->query($sql);
        if ($res && $res->num_rows > 0) {
    ...
    if ($english_page) {
        ...
        // Show progress
        error_log("Translating Terms page from English to {$target_language}");
        
        // Translate all fields except slug
        ...
        // Prepare translated values but DON'T SAVE yet
        $translated_data = [
            ...
            'slug' => $page ? $page['slug'] : 'terms' // Keep slug as-is
        ];
        ...
        $error = 'No English content found to translate. Please ensure English (ID 41) Terms page exists.';
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_page'])) {
    if ($page) {
        ...
        $sql = "DELETE FROM language_pages WHERE id=$delete_page_id AND language_id=$lang_id AND (LOWER(page_name) LIKE '%terms%' OR LOWER(slug) LIKE '%terms%')";
        if ($conn->query($sql)) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $page_name = trim($_POST['page_name'] ?? 'Terms');
    $slug = trim($_POST['slug'] ?? 'terms');
...
?>
    <div class="main-content" id="mainContent">
        ...
        <div class="page-section">
            <h4>Terms Page (<?php echo htmlspecialchars($lang['name']); ?>)</h4>
            <!--<div class="url-preview">URL: <b>/<?php echo htmlspecialchars($lang_code); ?>/<?php echo htmlspecialchars($page['slug'] ?? 'terms'); ?></b></div>-->
            <?php if ($success): ?>
            ...
            <form method="post">
                <div class="mb-3" style="display:none">
                    ...
                    <input type="text" class="form-control" name="page_name" value="<?php echo htmlspecialchars($page['page_name'] ?? 'Terms'); ?>">
                </div>
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" name="slug" value="<?php echo htmlspecialchars($translated_data ? $translated_data['slug'] : ($page['slug'] ?? ($english_page['slug'] ?? 'terms'))); ?>" placeholder="<?php echo htmlspecialchars($english_page['slug'] ?? 'terms'); ?>">
                </div>
                <div class="mb-3">
                ...
                <div class="d-flex gap-2">
                    ...
                    <?php endif; ?>
                    <?php if ($page): ?>
                        <button type="submit" name="delete_page" class="btn btn-danger" onclick="return confirm('Are you sure you want to delete this Terms page? This action cannot be undone.')">Delete Page</button>
                    <?php endif; ?>
                </div>
            </form>
        </div>
