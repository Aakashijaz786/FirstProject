...
// if (substr($original_uri, -2) === '//' || strpos($original_uri, '//') !== false) {
//     // ❌ If URL ends with // OR contains // anywhere → send 404
...
if (count($segments) > 0) {
    ...
    if (!$inactive_redirect_found) {
        ...
        $stmt = $conn->prepare("SELECT hp.id, hp.language_id, l.code, l.how_enabled FROM how_pages hp 
        ...
        if ($stmt === false) {
            ...
            $stmt->bind_param('s', $potential_slug);
            $stmt->execute();
            $stmt->bind_result($how_page_id, $how_lang_id, $how_lang_code, $how_enabled);
            if ($stmt->fetch()) {
                $stmt->close();
                // Check if how page is enabled for this language
                if (!$how_enabled) {
                    http_response_code(404);
                    include '404.php';
                    exit;
        ...
        $stmt = $conn->prepare("SELECT mp.id, mp.language_id, l.code, l.mp3_enabled FROM mp3_pages mp 
        ...
        if ($stmt === false) {
            ...
            $stmt->bind_param('s', $potential_slug);
            $stmt->execute();
            $stmt->bind_result($mp3_page_id, $mp3_lang_id, $mp3_lang_code, $mp3_enabled);
            if ($stmt->fetch()) {
                $stmt->close();
                // Check if mp3 page is enabled for this language
                if (!$mp3_enabled) {
                    http_response_code(404);
                    include '404.php';
                    exit;
        ...
        $stmt = $conn->prepare("SELECT sp.id, sp.language_id, l.code, l.stories_enabled FROM stories_pages sp 
        ...
        if ($stmt === false) {
            ...
            $stmt->bind_param('s', $potential_slug);
            $stmt->execute();
            $stmt->bind_result($stories_page_id, $stories_lang_id, $stories_lang_code, $stories_enabled);
            if ($stmt->fetch()) {
                $stmt->close();
                // Check if stories page is enabled for this language
                if (!$stories_enabled) {
                    http_response_code(404);
                    include '404.php';
                    exit;
...
// FAQ pages should always redirect to 404 (check before anything else)
if ($slug === 'faq' || $slug === 'faqs' || stripos($slug, 'faq') !== false) {
    http_response_code(404);
    include '404.php';
    exit;
}

// Copyright pages should always redirect to 404
if ($slug === 'c1-py' || stripos($slug, 'copyright') !== false) {
    http_response_code(404);
    include '404.php';
    exit;
}

// Contact page is only available for English (default language)
if (($slug === 'contact' || stripos($slug, 'contact') !== false) && $lang_code !== $default_lang['code']) {
    http_response_code(404);
    include '404.php';
    exit;
}
...
if (!empty($page_type) && !empty($base_slug)) {
    ...
    // Find the active slug for this page type in the current language and check if page is enabled
    switch ($page_type) {
        case 'mp3':
            $stmt = $conn->prepare("SELECT mps.slug, l.mp3_enabled FROM mp3_page_slugs mps 
            ...
            break;
        case 'stories':
            $stmt = $conn->prepare("SELECT sps.slug, l.stories_enabled FROM stories_page_slugs sps 
            ...
            break;
        case 'how':
            $stmt = $conn->prepare("SELECT hps.slug, l.how_enabled FROM how_page_slugs hps 
    ...
    if ($stmt) {
        ...
        // Bind result based on page type
        switch ($page_type) {
            case 'mp3':
                $stmt->bind_result($active_page_slug, $mp3_enabled);
                break;
            case 'stories':
                $stmt->bind_result($active_page_slug, $stories_enabled);
                break;
            case 'how':
                $stmt->bind_result($active_page_slug, $how_enabled);
                break;
        }
        ...
        if ($stmt->fetch()) {
            ...
            // Check if page is enabled for this language
            $page_enabled = false;
            switch ($page_type) {
                case 'mp3':
                    $page_enabled = $mp3_enabled;
                    break;
                case 'stories':
                    $page_enabled = $stories_enabled;
                    break;
                case 'how':
                    $page_enabled = $how_enabled;
                    break;
            }
