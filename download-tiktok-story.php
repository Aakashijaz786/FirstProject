...
// Fetch meta title, meta description, and active slug from stories_pages and stories_page_slugs for current language
...
// Get stories_pages row for current language
$res = $conn->query("SELECT id, meta_title, meta_description, page_name FROM stories_pages WHERE language_id=$current_lang_id LIMIT 1");
$stories_page_id = null;
$page_name = '';
if ($res && $res->num_rows > 0) {
    $row = $res->fetch_assoc();
    $stories_page_id = $row['id'];
    if (!empty($row['meta_title'])) $meta_title = $row['meta_title'];
    if (!empty($row['meta_description'])) $meta_description = $row['meta_description'];
    $page_name = $row['page_name'];
}
// Get active slug for this stories_page_id
if ($stories_page_id) {
    $res2 = $conn->query("SELECT slug FROM stories_page_slugs WHERE stories_page_id=$stories_page_id AND status='active' LIMIT 1");
...
foreach ($languages as $lang) {
    ...
    // Get stories_pages row for this language
    $res3 = $conn->query("SELECT id FROM stories_pages WHERE language_id = $lang_id LIMIT 1");
    $stories_page_id_alt = null;

    if ($res3 && $res3->num_rows > 0) {
        $row3 = $res3->fetch_assoc();
        $stories_page_id_alt = $row3['id'];
    }

    // Get active slug for this stories_page_id
    if ($stories_page_id_alt) {
        $res4 = $conn->query("SELECT slug FROM stories_page_slugs WHERE stories_page_id = $stories_page_id_alt AND status = 'active' LIMIT 1");
    ...
    // Add alternate link only if multiple languages
    if ($language_count > 1) {
        $alternate_links[] = [
            'url' => $url,
            'hreflang' => $lang_code
...
// Treat the entire slug as one unit (e.g., "es1/download-tiktok-stories")
if (!$lang) {
    // Search for the exact slug in stories_page_slugs table
    $res = $conn->query("SELECT sps.stories_page_id, l.* FROM stories_page_slugs sps 
                         JOIN stories_pages sp ON sps.stories_page_id = sp.id 
                         JOIN languages l ON sp.language_id = l.id 
                         WHERE sps.slug = '" . $conn->real_escape_string($current_slug) . "' 
...
// Fetch the active slug for the current language from stories_page_slugs, then fetch the stories page by stories_page_id
$stories_page = null;
$active_stories_slug = null;
if ($lang_id) {
    // Find the active slug and its stories_page_id for this language
    $sql = "SELECT sps.slug, sps.stories_page_id FROM stories_page_slugs sps
            JOIN stories_pages sp ON sp.id = sps.stories_page_id
            WHERE sp.language_id = $lang_id AND sps.status = 'active' ORDER BY sps.id DESC LIMIT 1";
    $res = $conn->query($sql);
    if ($res && $res->num_rows > 0) {
        $row = $res->fetch_assoc();
        $active_stories_slug = $row['slug'];
        $stories_page_id = $row['stories_page_id'];
        // Fetch the stories page by stories_page_id
        $res2 = $conn->query("SELECT * FROM stories_pages WHERE id = $stories_page_id LIMIT 1");
        if ($res2 && $res2->num_rows > 0) {
            $stories_page = $res2->fetch_assoc();
        }
    }
}
...
$page_header = h($stories_page['header'] ?? 'Download TikTok stories');
$page_heading = h($stories_page['heading'] ?? 'Download TikTok videos online');
$page_description = $stories_page['description'] ?? 'company name is a free TikTok download tool that helps you download TikTok videos without watermark online. Save TT videos with the highest quality in an MP4 file format and HD resolution. To find out how to use the TikTok watermark remover, follow the instructions below.';

// Fetch TikTok download instructions for the current language
$tiktok_instructions = [];

if ($lang_id) {
    $stmt = $conn->prepare("SELECT title, description FROM language_stories_titles WHERE language_id = ?");
